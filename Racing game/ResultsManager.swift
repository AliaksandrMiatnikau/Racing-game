...
class ResultsManager {
    ...
    static func saveResults(result: Results) {
    ...
    static func loadData() -> [Results]? {
        ...
        guard let result = try? JSONDecoder().decode([Results].self, from: content) else {
            return nil
        }
        return result
    }
